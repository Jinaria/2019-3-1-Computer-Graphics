...
def exp(rv):
    ...
    costh = np.cos(th)
    sinth = np.sin(th)
    R = np.array([[costh + nrv[0]*nrv[0]*(1-costh), nrv[0]*nrv[1]*(1-costh)-nrv[2]*sinth, nrv[0]*nrv[2]*(1-costh)+nrv[1]*sinth],
                  [nrv[1]*nrv[0]*(1-costh)+nrv[2]*sinth, costh+nrv[1]*nrv[1]*(1-costh), nrv[1]*nrv[2]*(1-costh)-nrv[0]*sinth],
                  [nrv[2]*nrv[0]*(1-costh)-nrv[1]*sinth, nrv[2]*nrv[1]*(1-costh)+nrv[0]*sinth, costh+nrv[2]*nrv[2]*(1-costh)]])
    
    return R

def log(R):
    rv = np.zeros(3)
    th0 = np.arccos((R[0, 0] + R[1, 1] + R[2, 2] - 1) / 2)
    # th1 = np.degrees(th0)
    rv[0] = (R[2, 1] - R[1, 2]) / (2 * np.sin(th0))
    rv[1] = (R[0, 2] - R[2, 0]) / (2 * np.sin(th0))
    rv[2] = (R[1, 0] - R[0, 1]) / (2 * np.sin(th0))
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'2014005187-10-1', None,None)
    if not window:
        glfw.terminate()
        return
